...
function parseAnalysis(report) {
  if (report?.rawResponse && typeof report.rawResponse === 'string') {
    try {
      ...
      return {
        ...
        improvements: Array.isArray(parsed.improvements) ? parsed.improvements : report.improvements || [],
        keyMoments: Array.isArray(parsed.keyMoments) ? parsed.keyMoments : report.keyMoments || [],
        trainingPlan: Array.isArray(parsed.trainingPlan) ? parsed.trainingPlan : report.trainingPlan || [],
        nextSteps: Array.isArray(parsed.nextSteps) ? parsed.nextSteps : report.nextSteps || [],
      };
    } catch {
      // fall through
    }
  ...
  return {
    ...
    keyMoments: Array.isArray(report?.keyMoments) ? report.keyMoments : [],
    trainingPlan: Array.isArray(report?.trainingPlan) ? report.trainingPlan : [],
...
export function buildParentReportModel(report) {
  const data = parseAnalysis(report || {});
